...
task SamToFastqAndBwaMemAndMba {
  ...
  command <<<
    ...
    if [ -s ${ref_alt} ]; then
      ...
      ${tool_path}/${bwa_commandline} /dev/stdin > ${output_bam_basename}.bam
...
task SamtoolsSort {
  ...
  command <<<
    set -o pipefail
    set -e
    ${tool_path}/samtools/samtools sort -T $TMPDIR/${output_bam_basename}.bam.tmp -m ${mem_limit} --threads ${samtools_threads} -l ${compression_level} ${input_bam} -o ${output_bam_basename}.bam
...
task GatherBqsrReports {
  ...
  command <<<
    export GATK_LOCAL_JAR=${tool_path}/gatk-jar && \
    ${tool_path}/gatk/gatk --java-options "-Dsamjdk.compression_level=${compression_level} -Xmx${java_heap_memory_initial}" \
      GatherBQSRReports \
...
task HaplotypeCaller {
  ...
  command <<<
      export GATK_LOCAL_JAR=${tool_path}/gatk-jar && \
      ${tool_path}/gatk/gatk --java-options -Xmx${haplotypecaller_java_heap_memory_initial} \
      HaplotypeCaller \
      -R ${ref_fasta} \
      -I ${input_bam} \
...
task CramToBam {
  ...
  command <<<
    ...
    ${tool_path}/samtools/samtools view -h -T ${ref_fasta} ${cram_file} |
    ${tool_path}/samtools/samtools view -b -o ${output_basename}.bam -
    ${tool_path}/samtools/samtools index -b ${output_basename}.bam
...
workflow PairedEndSingleSampleWorkflow {
  ...
  String bwa_commandline="minimap2-master/minimap2 -ax sr -L -K 100000000 -t $bwa_threads $bash_ref_fasta"
  ...
  call SamToFastqAndBwaMemAndMba {
    ...
    Float mapped_bam_size = size(AddReadGroup.output_bam, "GB")
  ...
  call SamtoolsSort as SortSampleBam {
    input:
      input_bam = MarkDuplicates.output_bam,
      output_bam_basename = base_file_name + ".aligned.duplicate_ARRG_marked.sorted",
